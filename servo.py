...
def main():
    ...
    mav = mavutil.mavlink_connection("/dev/ttyUSB0", baud=115200)  # Update with your connection string and baud rate

    # Wait for a heartbeat before sending commands
    mav.wait_heartbeat()    
    print(f"Heartbeat received from system (system_id: {mav.target_system} component_id: {mav.target_component})")
